...
def create_parser():
    ...
    parser.add_argument(
        "-v",
        "--verbose",
        help="Verbosity level in 10-DEBUG, 20-INFO, 30-WARNING, or 40-ERROR.",
        type=int,
        default=20,
        choices=(1, 10, 20, 30, 40),
    )

    sub_parsers = parser.add_subparsers(
        dest="command",
        help="[run] run simulation routine; "
        "[plot] plot results; "
    ...
    run.add_argument(
        "filename",
        help="Case file name. Power flow is calculated by default.",
        nargs="*",
    )
    run.add_argument(
        "-r",
        ...
        default=("pflow",),
        action="store",
        help="Simulation routine(s). Single routine or multiple separated with "
    ...
    run.add_argument(
        "--dime-address",
        help="DiME streaming server protocol, address and port,"
        "e.g., tcp://127.0.0.1:5000 or ipc:///tmp/dime2",
        type=str,
    )
    run.add_argument("--tf", help="End time of time-domain simulation", type=float)
    run.add_argument(
        "--qrt", help="Enable quasi-real-time stepping", action="store_true"
    )
    run.add_argument(
        "--kqrt",
        help="Scaling factor for quasi-real-time; e.g., kqrt=2 means the wall-clock time "
        "to complete a simulation is 2x the time of that simulation",
        type=float,
    )
    run.add_argument(
        "-c", "--convert", help="Convert to format.", type=str, default="", nargs="?"
    )
    run.add_argument(
        "-b",
        "--add-book",
        help="Add a template workbook for the specified model.",
        type=str,
    )
    run.add_argument(
        "--convert-all",
        help="Convert to format with all templates.",
        type=str,
        default="",
        nargs="?",
    )
    run.add_argument(
        "--state-matrix",
        help="Export state matrix to a .mat file. Need to run with `-r eig`",
        action="store_true",
    )
    run.add_argument("--profile", action="store_true", help="Enable Python cProfiler")
    run.add_argument(
        "-s", "--shell", action="store_true", help="Start in IPython shell"
    )
    run.add_argument("--no-preamble", action="store_true", help="Hide preamble")
    run.add_argument(
        "--no-pbar", action="store_true", help="Hide progress bar for time-domain"
    )
    run.add_argument(
        "--flat", action="store_true", help="Run no-disturbance (flat) simulation"
    )
    run.add_argument(
        "--pool",
        action="store_true",
        help="Start multiprocess with Pool and return a list of Systems",
    )
    run.add_argument(
        "--from-csv", help="Use data from a CSV file instead of from simulation"
    )
    run.add_argument(
        "-O",
        "--config-option",
    ...
    run.add_argument(
        "--init",
        help="Initialize variables only for time-domain simulation without running",
        action="store_true",
    )
    ...
    plot.add_argument(
        ...
        nargs=1,
        default=[],
        help="simulation output file name, which should end "
    ...
    plot.add_argument(
        ...
        nargs="?",
        type=int,
        help="the X-axis variable index, typically 0 for Time",
        default="0",
    )
    plot.add_argument(
        "y",
        nargs="*",
        help="Y-axis variable indices. Space-separated indices or a "
        "colon-separated range is accepted",
    )
    plot.add_argument(
        "--xmin", type=float, help="minimum value for X axis", dest="left"
    )
    plot.add_argument(
        "--xmax", type=float, help="maximum value for X axis", dest="right"
    )
    ...
    find_or_xargs.add_argument(
        "--find", type=str, help="find variable indices that matches the given pattern"
    )
    find_or_xargs.add_argument(
        "-a",
        "--xargs",
    ...
    plot.add_argument(
        "--exclude", type=str, help="pattern to exclude in find or xargs results"
    )
    ...
    plot.add_argument(
        ...
        "--savefig",
        action="store_true",
        help="save figure. The default format is `png`",
    )
    plot.add_argument(
        "-f",
        "--format",
    ...
    plot.add_argument(
        "-c", "--to-csv", help="convert npy output to csv", action="store_true"
    )
    ...
    doc.add_argument(
        "attribute", help="System attribute name to get documentation", nargs="?"
    )
    ...
    prep.add_argument(
        "-c",
        "--compile",
        help="compile the code with numba after codegen",
        action="store_true",
        dest="precompile",
    )
    ...
    prep.add_argument(
        "--incubate",
        help="Save generated pycode under the ANDES code directory to avoid codegen",
        action="store_true",
    )
    ...
    quick_or_extra.add_argument(
        "-q", "--quick", action="store_true", help="quick selftest by skipping codegen"
    )
    quick_or_extra.add_argument(
        "-e",
        "--extra",
        action="store_true",
        help="run all tests including interop (pandapower, gridcal, matpower)",
    )
